...
export function SmartCardFilterCriteria({ criteria }: { criteria: string[] }) {
  if (!criteria.length) return null
  return (
    <>
      <section aria-label="Locked smart-card criteria" className="rounded-xl border border-[#f5da77] bg-[#fff8db] p-4">
        <h3 className="flex items-center gap-2 text-xs font-extrabold text-[#475467]">
          <svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
            <rect x="5" y="10" width="14" height="11" rx="2" />
            <path d="M8 10V7a4 4 0 0 1 8 0v3" />
          </svg>
          Smart-card criteria · Locked
        </h3>
        ...
        <p className="mt-3 text-xs leading-5 text-[#475467]">These rules stay in place for this smart card.</p>
      </section>
