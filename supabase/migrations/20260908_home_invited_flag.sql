-- Return the Community Group invitation flag with the Home dashboard response.
-- This removes one follow-up query when Home loads. It changes no tables or
-- student data. Run this once in the Supabase SQL Editor before deploying the
-- matching app change.
...
AS $home_dashboard_with_invited_flag$
declare
...
begin
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'invited_to_community_group', recent.invited_to_community_group
      )
  ...
  from (
    select
      ...
      exists (
        select 1
        from public.follow_up_events invited_event
        where invited_event.contact_id = c.id
          and invited_event.event_type = 'interaction'
          and invited_event.invited_to_community_group = true
...
$home_dashboard_with_invited_flag$;
