...
export default async function GroupPage({ params, searchParams }: { params: Promise<{ groupId: string }>; searchParams: Promise<{ date?: string; tab?: string; filter?: string }> }) {
  ...
  const tab = ['people', 'history', 'settings'].includes(query.tab ?? '') ? query.tab : 'attendance'
